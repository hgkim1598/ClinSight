...
## 1. API 응답 계약 (프론트엔드 타입 기준)

### 핵심 원칙

백엔드 API는 **반드시 [`src/types/index.ts`](../src/types/index.ts)에 정의된 타입 형태 그대로 응답**해야 한다. 컴포넌트는 services 레이어가 반환하는 타입을 그대로 소비하므로, DynamoDB의 raw row 형태가 아닌 **프론트엔드가 기대하는 인터페이스로 변환된 응답**이 필요하다.

DynamoDB 저장 필드명/구조는 정규화·쿼리 효율을 위해 프론트 타입과 다를 수 있으며, **변환은 API Gateway 뒤의 Lambda가 담당**한다.
...
API Gateway
   ↑
Lambda (DTO 변환 계층)   ← 핵심 책임 위치
   ↓ DDB SDK
DynamoDB (정규화된 flat row)
```

- **저장 시**: Lambda가 프론트 타입 → DDB 속성으로 매핑
- **조회 시**: Lambda가 DDB row(s) → 프론트 타입으로 조립
- **여러 테이블 join**: Vitals+Labs, Departments+Staff 등은 Lambda에서 병렬 조회 후 합성

### 필드명 매핑 (Frontend → DynamoDB)

차이가 있는 필드만 나열. 동일한 필드(`name`, `age`, `tone` 등)는 생략.

#### Patient
| Frontend | DynamoDB | 비고 |
|---|---|---|
| `id` | `patientId` | DDB PK 명확화 |
| `admit` | `admitTime` | 시각 ISO 8601로 변환 |
| `diag` | `diagnosis` | 약어 → 명시적 |

#### LabDot (`Labs` 테이블)
| Frontend | DynamoDB | 비고 |
|---|---|---|
| `time` | `timestamp` | ISO 8601 |
| `type` | `labType` | enum 키명 |
| (—) | `sortKey` | DDB 합성 SK (`timestamp#labType`) |

#### VitalKey (`Vitals` 테이블)
| Frontend | DynamoDB | 비고 |
|---|---|---|
| `urine_output` | `urineOutput` (measurementType / attribute) | 프론트 VitalKey의 `urine_output`(snake_case) → DynamoDB Vitals의 `measurementType: urineOutput`(camelCase). 와이드 포맷에서는 attribute 이름, 톨 포맷 대안에서는 `measurementType` 값으로 사용 |
| `hr` / `map` / `spo2` / `rr` / `temp` / `gcs` | 동일 | 단일 토큰이라 변환 불필요 |

#### Alert (nested patient → flat 저장)
| Frontend | DynamoDB | 비고 |
|---|---|---|
| `id` | `alertId` | |
| `timestamp` | `createdAt` | ISO 8601 + GSI SK |
| `patient.id` | `patientId` | nested → flat (Lambda가 응답 시 nested 복원) |
| `patient.name` | `patientName` | 역정규화 |
| `patient.bed` | `patientBed` | 역정규화 |
...
| `Department.members[]` | (별도 `Staff` 테이블) | Lambda가 GSI 조회 후 nested 조립 |
| `StaffMember.id` | `staffId` | |
| (—) | `Staff.departmentId` | Staff에 부서 FK 추가 필드 |

#### ConsultationRequest
| Frontend | DynamoDB | 비고 |
|---|---|---|
| `id` | `consultationId` | |
| 그 외 | 동일 | `patientId`, `requestedAt` 등 그대로 |

#### ChatMessage
| Frontend | DynamoDB | 비고 |
|---|---|---|
| `id` | `messageId` | |
| (—) | `sessionId`, `timestamp` | DDB 키 추가 필드 |

#### PatientReport
| Frontend | DynamoDB | 비고 |
|---|---|---|
| `generatedAt` (Date) | `generatedAt` (S, ISO 8601) | Date 객체 ↔ ISO 변환 |
| `patient` (nested) | (snapshot으로 평탄화 또는 그대로 Map) | 참조 vs 동결 스냅샷 |
| `vitals[]`, `labs[]`, `predictions[]` | (Map의 List로 그대로 저장) | 보고서 시점 동결 |

### Lambda 변환 계층 책임 요약

| 변환 종류 | 예시 |
|---|---|
| 필드명 rename | `patientId` ↔ `id` (Patient) |
| nested ↔ flat | `Alert.patient` 분해/재조립 |
| Multi-row → single object | `Vitals` timestamp rows → `VitalData.series` 집계 |
| Multi-row → keyed map | `ModelPredictions` 5 rows → `Record<ModelKey, ModelPrediction>` |
| Cross-table join | `Department + Staff` → `Department.members[]` |
| Date ↔ ISO string | `PatientReport.generatedAt` |
| 상대 시각 ↔ 절대 시각 | `TrendPoint.t = '-6h'` ↔ DDB ISO timestamp (둘 중 source 결정 필요) |
...
### 데이터 표현
- **시각 표기**: 모든 timestamp는 **DynamoDB에 ISO 8601 KST(`2026-04-29T14:20:00+09:00`)로 저장**. 프론트에 내려줄 때 상대 시간(`-6h` 등)으로 변환은 Lambda 또는 프론트 포맷터에서 처리. mock의 표시용 문자열(`"14:20"`, `"2026-04-21 08:14"`, `"-6h"`)은 변환 대상.
...
### 저장 모델 vs API 응답 모델
- **저장**: DynamoDB는 정규화된 flat row. 쿼리 효율(PK/SK/GSI)에 최적화된 키 명명 사용.
- **응답**: API Gateway 뒤 Lambda가 프론트 타입에 맞는 중첩/집계 응답으로 변환.
- **변환 책임은 Lambda에 집중** — 프론트 컴포넌트는 services 시그니처만 신뢰. 단일 endpoint가 다중 테이블을 join해야 할 수도 있음(예: `GET /patients/{id}/vitals`은 Vitals+Labs를 동시 조회 후 `VitalData`로 합성).
...
### 💡 GSI 개선 대안 (Hot partition 회피)

`risk` 값은 3종(high/med/low)뿐이라 GSI PK로 사용 시 **저카디널리티로 hot partition 위험**이 있음. 다중 ICU 환경에서 더 균등 분포가 필요하면 다음 합성 PK 권장:

| 대안 GSI | PK | SK | 비고 |
|---|---|---|---|
| `IcuRiskAdmitIndex` | `icuId#risk` (예: `ICU-01#high`) | `admitTime` | ICU별로 위험도 분리 → partition 분산 |

단일 ICU 운영(현재 mock 기준 `ICU-01` 단일)이라면 기존 `risk` PK도 무방하나, 멀티 ICU 확장 시 위 패턴 권장.
...
| `icuId` | S | ✓ | ICU 식별자 (멀티 ICU 시 필수, 단일 ICU도 명시 권장) |

### DDB 저장 예시 ([patients.ts:4-16](../src/api/mock/patients.ts#L4-L16))
```json
{
  "patientId": "PT-19482",
  "name": "김영호",
...
### API 응답 형태
- **Service**: `getPatients(): Patient[]`, `getPatientById(id: string): Patient | undefined`
- **반환 타입**: [`Patient`](../src/types/index.ts) (단일 또는 배열)
- **Lambda 변환**: DDB row → `Patient`
  - rename: `patientId → id`, `admitTime → admit`, `diagnosis → diag`
  - drop: `icuId` (프론트 타입에 없음, 멀티 ICU UI 도입 시 추가)
...
### DDB 저장 예시 ([vitals.ts:13](../src/api/mock/vitals.ts#L13))
```json
{
  "patientId": "PT-19482",
...
### API 응답 형태
- **Service**: `getVitals(patientId: string): VitalData`
- **반환 타입**: [`VitalData`](../src/types/index.ts) = `{ series: Record<VitalKey, VitalSeries>, labs: LabDot[] }`
- **Lambda 변환**: 두 테이블 join + 집계
  1. `Vitals` 테이블 Query → timestamp별 row 집합
  2. `Labs` 테이블 Query (§6) → LabDot 리스트
  3. **시계열 집계**: 각 `VitalKey`(hr/map/...)별로 row들을 순회하며 `data: number[]` + `times: string[]` 페어로 풀어냄. 결측 row는 `data`에서 누락 처리(또는 `null` 채움).
  4. **메타 결합**: `VitalSeries.label` / `unit` / `normal` 은 마스터 테이블 또는 Lambda 상수에서 주입 (예: `hr → { label: 'Heart Rate', unit: 'bpm', normal: [60, 100] }`).
  5. `labs[]`은 `LabDot[]` 형태로 통과(필드 rename 포함, §6 참조).
...
### 💡 SK 개선 대안 (특정 labType 빠른 조회)

자주 쓰이는 패턴이 "환자 X의 Cre만 추적"처럼 **특정 labType만** 조회하는 경우라면 SK 합성 순서를 뒤집는 것이 효율적:

| 대안 SK 합성 | 쿼리 효율 |
|---|---|
| `labType#timestamp` (예: `cre#2026-04-25T14:00:00+09:00`) | `Query patientId AND begins_with(SK, 'cre#')` 가능 — FilterExpression 불필요 |
| 기존 `timestamp#labType` | 시간 범위 + 모든 labType 조회에 유리, 단일 type 조회는 Filter 필요 |

> 둘 중 어느 쿼리가 빈번한지에 따라 선택. 두 패턴 모두 빈번하면 GSI를 추가해 양방향 지원.
...
### DDB 저장 예시 ([vitals.ts:60](../src/api/mock/vitals.ts#L60))
```json
{
  "patientId": "PT-19482",
...
### API 응답 형태
- **단독 endpoint 없음** — `getVitals()` 응답의 `VitalData.labs[]`에 포함되어 함께 반환.
- **반환 타입**: [`LabDot`](../src/types/index.ts) = `{ time, label, value, type }`
- **Lambda 변환**: row → `LabDot`
  - rename: `timestamp → time`, `labType → type`
  - drop: `sortKey` (DDB 키만, 응답 불필요)
...
| `modelVersion` | S | ✓ | 모델 버전 (예: `mortality-v1.2.0`) — A/B 비교, 모델 모니터링 필수. 백엔드 예측 생성 시 자동 채움. 프론트 타입에는 없는 내부 메타 필드. 기본값: `'1.0.0'` |

### DDB 저장 예시 ([models.ts:16-49](../src/api/mock/models.ts#L16-L49))
```json
{
  "patientId": "PT-19482",
  "modelKey": "mortality",
  ...
  "updatedAt": "2026-04-29T14:00:00+09:00",
  "modelVersion": "mortality-v1.2.0"
}
```
...
### API 응답 형태
- **Service**: `getModelPredictions(patientId: string): Record<ModelKey, ModelPrediction>`
- **반환 타입**: [`Record<ModelKey, ModelPrediction>`](../src/types/index.ts) — modelKey를 키로 하는 객체
- **Lambda 변환**:
  1. `Query patientId` → 5개 row
  2. `reduce`로 `{ mortality: {...}, aki: {...}, ards: {...}, sic: {...}, shock: {...} }` 객체 조립
  3. 각 row의 `trend`/`shap`/`raw`/`llmSummary`/`escalation`은 그대로 통과 (필드명 동일)
  4. drop: `updatedAt`, `modelVersion`은 응답에 미포함(내부 메타) 또는 별도 헤더
  5. 환자 미존재 시 fallback: 5개 키 모두 `tone='safe'` + 빈 배열 (현재 mockService와 동일)
...
### DDB 저장 예시 ([sofaScores.ts:8-18](../src/api/mock/sofaScores.ts#L8-L18))
```json
{
  "patientId": "PT-19482",
...
### API 응답 형태
- **Service**: `getSofaTrend(patientId: string): SofaTrend`
- **반환 타입**: [`SofaTrend`](../src/types/index.ts) = `{ times: string[], scores: Record<OrganKey, Array<number | null>> }`
- **Lambda 변환**: row 시계열 → 장기별 배열로 pivot
  1. `Query patientId, SK between (now-24h, now)` → 시간순 row 배열
  2. `times[] = rows.map(r => r.timestamp)` (또는 상대 시각 `-24h`/`-21h`/...로 변환)
  3. `scores = { cardio: rows.map(r => r.cardio ?? null), resp: rows.map(r => r.resp ?? null), ... }`
  4. **결측 보존**: row에 attribute 없음 → 응답 배열에 `null`. 보간 금지(CLAUDE.md 규칙).
...
### DDB 저장 예시 ([staffing.ts:3-21](../src/api/mock/staffing.ts#L3-L21))
```json
{
  "icuId": "ICU-01",
...
### API 응답 형태
- **Service**: `getStaffing(): StaffingSnapshot`
- **반환 타입**: [`StaffingSnapshot`](../src/types/index.ts)
- **Lambda 변환**: 거의 없음. DDB row 그대로 통과 (필드명·구조 동일).
  - `doctors`/`nurses`/`thresholds` Map은 그대로 nested object로 응답.

---

## 10. `AiInsightsCache` (옵션)
...
- **반환 타입**: `string` (단순 텍스트)
- **Lambda 흐름**:
  1. `GetItem(insightKey = '{model}#{section}')`
  2. hit → `text` 반환
  3. miss → Bedrock 호출 → 결과를 `PutItem`(TTL 24h 설정) → `text` 반환
- **변환 거의 없음** — 단일 String. 캐시 메타(`generatedAt`, `ttl`)는 응답 미포함.
...
`status` 값은 3종(new/acknowledged/resolved)뿐. 단일 ICU에서 미확인 알림이 한 partition에 몰리면 **hot partition 위험**. 합성 PK 권장:

| 대안 GSI | PK | SK | 비고 |
|---|---|---|---|
| `IcuStatusCreatedAtIndex` | `icuId#status` (예: `ICU-01#new`) | `createdAt` | ICU별로 status partition 분산 |

알림 `Alert` 응답은 `icuId` 필드 없이도 가능하므로, DDB에는 `icuId` 추가 저장 + GSI 키로 사용, 응답 시 drop.
...
| `icuId` | S | ✓ | (개선 GSI 사용 시) ICU 식별자 |

### DDB 저장 예시 ([alerts.ts:18-33](../src/api/mock/alerts.ts#L18-L33))
```json
{
  "alertId": "alert-001",
  "patientId": "PT-19482",
...
- **Services**:
  ...
  - `acknowledgeAlert(id: string, by: string): Alert | null`
  - `resolveAlert(id: string): Alert | null`
- **반환 타입**: [`Alert`](../src/types/index.ts) (단일/배열) 또는 `number`(카운트)
- **Lambda 변환**: row → `Alert` 재구성
  - rename: `alertId → id`, `createdAt → timestamp`
  - **nested 복원**: `patientId` + `patientName` + `patientBed` → `patient: { id, name, bed }`
  - drop: `icuId` (GSI 키 용도, 응답 미포함)
  - `getNewAlertCount`: `Query Select=COUNT` 결과의 `Count` 값 그대로 응답
...
### DDB 저장 예시 ([timeline.ts:11-18](../src/api/mock/timeline.ts#L11-L18))
```json
{
  "patientId": "PT-19482",
...
### API 응답 형태
- **Service**: `getTimeline(patientId: string): TimelineEvent[]`
- **반환 타입**: [`TimelineEvent[]`](../src/types/index.ts)
- **Lambda 변환**: row → `TimelineEvent`
  - rename: `eventId → id`, `timestamp → time`
  - drop: `patientId`, `sortKey`
  - `time`은 ISO를 그대로 둘지 표시용 단축 형식(`"14:20"`)으로 변환할지 결정 필요 (현재 mock은 표시용)
...
### DDB 저장 예시 ([departments.ts:10-18](../src/api/mock/departments.ts#L10-L18))
```json
...
### API 응답 형태
- **Service**: `getDepartments(): Department[]`
- **반환 타입**: [`Department[]`](../src/types/index.ts) — 각 항목은 `{ id, name, members: StaffMember[] }`
- **Lambda 변환**: 두 테이블 join + nested 조립
  1. `Scan Departments` → 부서 row 배열
  2. 각 부서별로 `Query Staff GSI1 departmentId` → 의료진 row 배열
  3. **조립**: `Department[]`로 nested 결합
  4. **rename**:
     - `Departments.departmentId → Department.id`
     - `Staff.staffId → StaffMember.id`
  5. **drop**: `Staff.departmentId` (이미 nested 위치로 표현됨)
...
### DDB 저장 예시 ([departments.ts:11](../src/api/mock/departments.ts#L11))
```json
{
  "staffId": "staff-001",
  "name": "박지훈",
...
### API 응답 형태
- **단독 endpoint 없음** — Departments 응답에 nested(`Department.members[]`)로 포함되어 반환.
- **반환 타입**: [`StaffMember`](../src/types/index.ts) (Departments 응답 내부)
- **Lambda 변환**: row → `StaffMember`
  - rename: `staffId → id`
  - drop: `departmentId` (응답 위계로 이미 표현)
- 추후 staff inbox 등 단독 사용 시 `GET /staff/{id}` endpoint + 그대로 반환 정의 가능.
...
`status` 값(`pending`/`accepted`/`completed`)이 3종뿐이라 GSI2의 hot partition 위험. Alerts와 동일한 합성 PK 권장:

| 대안 GSI | PK | SK | 비고 |
|---|---|---|---|
| `IcuStatusRequestedAtIndex` | `icuId#status` | `requestedAt` | ICU별로 status 분산 |

추가로 **수신자별 inbox**는 GSI3(`StaffIdRequestedAtIndex` — recipients의 staffId를 별도 attribute로 평탄화)가 필요. 현재 `recipients`는 List of Map이라 GSI 키로 직접 사용 불가 → 평탄화 컬럼 추가 또는 단일 테이블 디자인 고려.
...
### DDB 저장 예시 ([consultations.ts:7-21](../src/api/mock/consultations.ts#L7-L21))
```json
{
...
### API 응답 형태
- **Services**:
  - `getConsultations(patientId?: string): ConsultationRequest[]`
  - `createConsultation(req): ConsultationRequest`
- **반환 타입**: [`ConsultationRequest`](../src/types/index.ts) (단일 또는 배열)
- **Lambda 변환**: row → `ConsultationRequest`
  - rename: `consultationId → id`
  - drop: `icuId` (GSI 키 용도, 응답 미포함)
  - `recipients` 그대로 (`L (M)` → `ConsultRecipient[]`)
  - **생성 흐름**: `createConsultation` 요청 시 Lambda가 ID 생성 + `requestedBy`(Cognito `sub`) + `requestedAt`(서버 시각) + `status='pending'` 부여 후 `PutItem`
...
`ChatContext`(types/index.ts):
- `{ type: 'section', section: AiInsightSection, modelKey: ModelKey }` — 섹션별 채팅
- `{ type: 'patient', patientId: string }` — 환자 전체 채팅

### 키 설계

| 속성 | 타입 | 역할 |
|---|---|---|
| `sessionId` | S | **PK** (예: `chat-PT-19482-patient-{nanoId}` 또는 `chat-PT-19482-section-mortality-trend-{nanoId}`) |
| `timestamp` | S | **SK** (메시지 순서) |

> 세션 ID 생성 규칙은 클라이언트가 채팅 패널 첫 진입 시 발급. context에 따라 prefix 다르게.
...
| `sessionId` | S | ✓ | PK |
| `timestamp` | S | ✓ | SK (ISO 8601) |
| `messageId` | S | ✓ | 메시지 식별자 (예: nano ID) |
| `role` | S | ✓ | `ai` / `user` |
| `text` | S | ✓ | 메시지 본문 |
| `contextType` | S | ✓ | `section` / `patient` (`ChatContext.type`) |
| `patientId` | S | ✗ | (context에 환자 정보가 있을 때) |
| `modelKey` | S | ✗ | (section 컨텍스트인 경우) |
| `section` | S | ✗ | `trend` / `shap` / `rawMetrics` / `auxiliary` (section 컨텍스트인 경우) |
| `ttl` | N | ✗ | 세션 종료 후 30일 자동 만료 (옵션) |

### DDB 저장 예시 (가상)
```json
{
  "sessionId": "chat-PT-19482-section-mortality-trend-abc123",
  "timestamp": "2026-04-29T14:21:00+09:00",
  "messageId": "msg-001",
  "role": "ai",
  "text": "최근 6시간 동안 사망 위험 확률이 빠르게 상승하는 패턴이 관찰됩니다...",
  "contextType": "section",
  "patientId": "PT-19482",
  "modelKey": "mortality",
  "section": "trend"
...
| 시나리오 | 동작 | API 매핑 |
|---|---|---|
| 세션 전체 대화 (시간순) | `Query sessionId` | (UI 채팅 패널 재진입 시 이력 로드) |
| 메시지 추가 | `PutItem` | `getChatResponse()` 호출 후 두 메시지(user+ai) 저장 |
| 인트로 메시지 1건만 | `GetItem(sessionId, timestamp=오프셋)` | `getChatIntro(context)` |

### API 응답 형태
- **Services**:
  - `getChatIntro(context: ChatContext): string` — 첫 진입 시 AI 인트로
  - `getChatResponse(context: ChatContext, userMessage: string): string` — user 메시지 → AI 응답
- **현재**: stateless string 반환. **백엔드 도입 시**: `ChatMessage` 단건 또는 세션 전체 배열 반환으로 확장될 수 있음.
- **Lambda 변환**: row → `ChatMessage`
  - rename: `messageId → id`
  - drop: `sessionId`, `timestamp`, `contextType`, `patientId`, `modelKey`, `section` (DDB 키/메타, 응답엔 `id`/`role`/`text`만 필요)
- **저장 흐름**: 사용자 메시지 들어오면 → ChatMessages 테이블에 user row 저장 → Bedrock 호출 → AI 응답 row 저장 → 응답 반환
...
|---|---|---|
| `patientId` | S | **PK** |
| `generatedAt` | S | **SK** (ISO 8601, 최신 보고서가 마지막) |
...
|---|---|---|---|
| `patientId` | S | ✓ | PK |
| `generatedAt` | S | ✓ | SK (생성 시각). DynamoDB에는 ISO 8601 문자열로 저장. 프론트의 `PatientReport.generatedAt`은 `Date` 타입이므로, Lambda에서 ISO string으로 반환하고 프론트 서비스에서 `new Date()`로 복원한다. |
| `reportId` | S | ✓ | 보고서 ID (UUID) — 단건 deeplink 용 |
| `generatedBy` | S | ✓ | 생성자 (Cognito `sub`) |
| `patientSnapshot` | M | ✓ | 보고서 시점의 `Patient` 동결 (참조 아닌 복사) |
| `vitals` | L (M) | ✓ | `ReportVitalRow[]` 동결 |
| `labs` | L (M) | ✓ | `ReportLabRow[]` 동결 |
| `predictions` | L (M) | ✓ | `ReportPrediction[]` 동결 |
| `consultationNotes` | L (M) | ✗ | 사용자가 추가한 협진 메모 (현재 PatientReportModal 내 로컬 상태인 것을 영속화) |

> **스냅샷 정책**: 보고서는 "그 시점의 상태"를 기록하므로, 환자가 사망/퇴실해도 보고서는 살아있어야 함. 따라서 외부 테이블 참조가 아닌 **동결된 복사본**을 저장.

### DDB 저장 예시 (가상)
```json
{
  "patientId": "PT-19482",
  "generatedAt": "2026-04-29T14:30:00+09:00",
  "reportId": "report-uuid-xxx",
  "generatedBy": "cognito-sub-xxx",
  "patientSnapshot": {
    "id": "PT-19482",
    "name": "김영호",
    "age": 72,
  ...
  "vitals": [
    { "key": "hr", "label": "Heart Rate", "unit": "bpm", "latestValue": 107.4, "latestTime": "Now", "normalRange": [60, 100], "status": "critical" }
    /* ... */
  ],
  "labs": [
    { "label": "Lactate", "value": "5.2", "unit": "mmol/L", "time": "Now", "normalRange": "0.5–2.2 mmol/L" }
    /* ... */
  ],
  "predictions": [
    { "key": "mortality", "title": "사망 위험", "probability": 74, "risk": "high" }
    /* ... */
  ],
  "consultationNotes": []
}
...
| 시나리오 | 동작 | API 매핑 |
|---|---|---|
| 환자 보고서 이력 (최신순) | `Query patientId` ScanIndexForward=false | `GET /patients/{id}/reports` (추후) |
| 단건 보고서 | `GetItem(patientId, generatedAt)` | `GET /patients/{id}/reports/{generatedAt}` |
| 보고서 신규 생성 | `PutItem` | `POST /patients/{id}/reports` ← `getPatientReport(id)` 결과를 저장 |

### API 응답 형태
- **Service**: `getPatientReport(patientId: string): PatientReport | null`
- **반환 타입**: [`PatientReport`](../src/types/index.ts) = `{ patient, generatedAt: Date, vitals[], labs[], predictions[] }`
- **현재 흐름**: services에서 조합형으로 빌드. 매 호출마다 즉석 생성.
- **백엔드 도입 후 흐름**: 두 가지 모드 가능
  1. **Live mode**: Lambda가 매번 patientService+vitalService+modelService 호출하여 최신 보고서 생성 (현재 mockService와 동일)
  2. **Persisted mode**: `PatientReports` 테이블에 저장된 가장 최근 보고서 반환 + 신규 요청 시 새 row 생성
- **Lambda 변환**: row → `PatientReport`
  - `generatedAt` (S, ISO 8601) → `Date` 객체 (또는 ISO 문자열 그대로 두고 프론트에서 `new Date()`)
  - `patientSnapshot` Map → `Patient` (필드명 그대로 동결되어 있다고 가정)
  - `vitals`/`labs`/`predictions` List → 각 타입 배열로 통과
...
| 인증 (Cognito) | `requestedBy`, `acknowledgedBy`, `generatedBy` 등은 현재 `'담당 의료진'` placeholder. Cognito `sub` 또는 staff ID로 교체 |
...
| 모델 모니터링 | `ModelPredictions.modelVersion` 추가됨. 버전별 성능 비교·rollout 로직 별도 설계 필요 |
| 채팅 세션 종료 시점 | `ChatMessages.ttl` 적용 기준 (UI에서 명시적 종료? 마지막 메시지 후 N일?) |
| 보고서 생성 빈도 | `PatientReports`를 매 조회 시 생성할지, 명시적 "보고서 발행" 버튼 시에만 생성할지 |
